...
function lsSet(userId: string, alerts: PriceAlert[]) {
  localStorage.setItem(`nexusai_alerts_v3_${userId}`, JSON.stringify(alerts));
  localStorage.setItem(`nexusai_alerts_init_${userId}`, '1');
}
...
const ASSETS = [
  ...
  { symbol: 'BTC',   label: 'Bitcoin (BTC)',        minVal: 1000, maxVal: 500000},
  { symbol: 'ETH',   label: 'Ethereum (ETH)',       minVal: 100,  maxVal: 50000 },
...
function validate(form: any): FormErrors {
  ...
  if (!form.target_value) {
    ...
    else if (asset && val < asset.minVal) errors.target_value = `Min $${asset.minVal.toLocaleString()} for ${assetName(form.symbol)}`;
...
function AlertsContent({ userId }: { userId: string }) {
  ...
  const persist = (next: PriceAlert[]) => { setAlerts(next); lsSet(userId, next); };
  ...
  useEffect(() => {
    ...
    alertsApi.getAlerts()
      .then(serverAlerts => {
        if (Array.isArray(serverAlerts)) { persist(serverAlerts); setBackendSync(true); }
        else { const s = lsGet(userId); persist(s !== null ? s : SEED); }
      })
      .catch(() => { const s = lsGet(userId); persist(s !== null ? s : SEED); })
      .finally(() => setLoading(false));
    alertsApi.getHistory().then(h => { if (Array.isArray(h)) setHistory(h); }).catch(() => {});
  }, [isAuthenticated]);
  ...
  async function createAlert() {
    ...
    setSubmitting(true);
    const payload = {
      symbol: form.symbol, asset_name: assetName(form.symbol), condition: form.condition,
      target_value: parseFloat(form.target_value),
      notify_email: form.notify_email, notify_sms: form.notify_sms,
      contact_email: form.contact_email.trim() || null,
      contact_phone: form.contact_phone.trim() || null,
    };
    const optimistic: PriceAlert = { id: `local-${Date.now()}`, ...payload, is_active: true, created_at: new Date().toISOString() } as PriceAlert;
    persist([optimistic, ...alerts]);
    setSuccess(`✓ Alert: ${payload.asset_name} ${CONDITION_LABELS[form.condition]} $${payload.target_value.toLocaleString()}`);
    ...
    if (isAuthenticated) {
      alertsApi.createAlert(payload)
        .then(async () => { const fresh = await alertsApi.getAlerts(); if (Array.isArray(fresh)) persist(fresh); })
        .catch(() => {});
    }
  }

  function toggleAlert(id: string, isActive: boolean) {
    persist(alerts.map(a => a.id === id ? { ...a, is_active: !isActive } : a));
    if (isAuthenticated && !isLocalId(id)) alertsApi.updateAlert(id, { is_active: !isActive }).catch(() => {});
  }

  function deleteAlert(id: string) {
    persist(alerts.filter(a => a.id !== id));
    if (isAuthenticated && !isLocalId(id)) alertsApi.deleteAlert(id).catch(() => {});
  }
  ...
  const CreateForm = (
    <div className="card h-fit">
      ...
      <div className="space-y-3">
        {/* Asset + Condition: side by side on mobile */}
          ...
          <div>
            ...
            {currentAsset && (
              <p className="text-[10px] text-gray-600 mt-1">
                Range: ${currentAsset.minVal.toLocaleString()} – ${currentAsset.maxVal.toLocaleString()}
              </p>
            )}
          </div>
          <div>
        ...
        <div>
          <label className="text-xs text-gray-400 mb-1 block font-medium">Target Value ($)</label>
          <input className={cn('input', formErrors.target_value ? 'border-red-400' : '')}
            type="number" step="0.01"
            placeholder={form.symbol === 'XAU' ? '3500' : form.symbol === 'BTC' ? '75000' : form.symbol === 'WTI' ? '58' : '900'}
        ...
        {/* Email + Phone: side by side if both shown */}
        {(form.notify_email || form.notify_sms) && (
          <div className={cn('grid gap-3', form.notify_email && form.notify_sms ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1')}>
            {form.notify_email && (
              <div>
        ...
        <button onClick={createAlert} disabled={submitting}
          className="btn-primary w-full mt-1 flex items-center justify-center gap-2 h-10 disabled:opacity-40">
          {submitting ? <div className="w-4 h-4 border-2 border-bg/30 border-t-bg rounded-full animate-spin" /> : <Bell size={14} />}
          Create Alert
        </button>
      ...
      <div className="mt-5 pt-4 border-t border-border">
        <p className="text-xs text-gray-500 font-semibold mb-1">How alerts work:</p>
        <p className="text-xs text-gray-600 leading-relaxed">
          When price hits your target, the backend sends via <strong className="text-gray-400">Resend</strong> (email) or <strong className="text-gray-400">Twilio</strong> (SMS). Checked every 3 min. Set your API keys in Railway.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-3 mb-1">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
          {isAuthenticated && backendSync
            ? <span className="flex items-center gap-1 text-[10px] font-mono font-bold px-2 py-1 rounded bg-green-400/10 text-green-400 border border-green-400/20 whitespace-nowrap"><Cloud size={10} /> Synced</span>
            : <span className="flex items-center gap-1 text-[10px] font-mono font-bold px-2 py-1 rounded bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 whitespace-nowrap"><HardDrive size={10} /> Local</span>
          }
          {/* Mobile: toggle form button */}
          <button onClick={() => setShowForm(v => !v)}
            className="xl:hidden btn-primary flex items-center gap-1.5 text-xs px-3 py-1.5">
            <Plus size={12} /> {showForm ? 'Hide' : 'New Alert'}
          </button>
        </div>
      </div>

      {/* Mobile: form toggled above list */}
      {showForm && <div className="xl:hidden mb-4">{CreateForm}</div>}

      {success && !showForm && (
        <div className="flex items-start gap-2 p-3 mb-4 bg-green-400/10 border border-green-400/20 rounded-lg xl:hidden">
          <CheckCircle2 size={14} className="text-green-400 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-green-400">{success}</p>
        </div>
      )}
      ...
      <div className="grid xl:grid-cols-2 gap-4 sm:gap-5">
        ...
        <div className="space-y-4 sm:space-y-5">
                  ...
                  <div key={a.id} className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-semibold truncate">{a.asset_name}</p>
                        <span className={cn('text-[9px] font-mono font-bold px-1 py-0.5 rounded border flex-shrink-0',
                          a.is_active ? 'text-green-400 bg-green-400/10 border-green-400/20' : 'text-gray-500 bg-bg-4 border-border')}>
          ...
          <div className="card">
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-2">
                {history.map(h => (
                  <div key={h.id} className="flex items-start gap-2 sm:gap-3 p-3 rounded-lg bg-yellow-400/5 border border-yellow-400/20">
                    <CheckCircle2 size={14} className="text-yellow-400 flex-shrink-0 mt-0.5" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-medium">{h.message}</p>
        ...
        {/* Right: form (desktop only — mobile is toggled above) */}
