...
export function Portfolio() {
  ...
  useEffect(() => {
    portfolioApi.getPortfolio()
      .then(data => { setPositions(data.positions); setSummary(data.summary); })
      .catch(() => { setPositions(DEMO_POSITIONS); setSummary(DEMO_SUMMARY); });
  }, []);

  const displayPositions = positions.length > 0 ? positions : DEMO_POSITIONS;
  const displaySummary = summary || DEMO_SUMMARY;
  const totalVal = displaySummary.total_value;

  const pieData = displayPositions.map(p => ({
    name: p.name, value: Math.round((p.market_value / totalVal) * 100), color: p.color,
  }));
  ...
  async function addPosition() {
    ...
    try {
      await portfolioApi.addPosition({ ...form, quantity: parseFloat(form.quantity), avg_cost: parseFloat(form.avg_cost) });
      const data = await portfolioApi.getPortfolio();
      setPositions(data.positions); setSummary(data.summary);
    } catch { /* demo mode */ }
  ...
  return (
    <div>
      ...
      {showAdd && (
        <div className="card mb-5 border-gold/30">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={addPosition} disabled={loading} className="btn-primary">Add Position</button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {[
          { label: 'Total Value', value: `$${displaySummary.total_value.toLocaleString()}`, color: 'text-gold' },
          { label: 'Total P&L', value: `${displaySummary.total_pnl >= 0 ? '+' : ''}$${displaySummary.total_pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, color: displaySummary.total_pnl >= 0 ? 'text-green-400' : 'text-red-400' },
          { label: 'Return %', value: `${displaySummary.total_pnl_pct >= 0 ? '+' : ''}${displaySummary.total_pnl_pct.toFixed(2)}%`, color: displaySummary.total_pnl_pct >= 0 ? 'text-green-400' : 'text-red-400' },
          { label: 'Positions', value: `${displayPositions.length}`, color: 'text-violet-400' },
        ].map(s => (
          <div key={s.label} className="card">
        ...
        <div className="card lg:col-span-3">
          ...
          <div className="space-y-0">
            {displayPositions.map(p => (
              <div key={p.id} className="flex items-center gap-3 py-3 border-b border-border last:border-0 hover:bg-bg-3 -mx-3 px-3 rounded-lg transition-all">
                <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: p.color }} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold">{p.name}</p>
                  <p className="text-xs text-gray-500 font-mono">{p.quantity} × ${p.avg_cost}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold font-mono">${p.market_value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                  <p className={cn('text-xs font-mono', p.pnl >= 0 ? 'text-green-400' : 'text-red-400')}>
                    {p.pnl >= 0 ? '+' : ''}${p.pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })} ({p.pnl_pct.toFixed(1)}%)
                  </p>
                </div>
              </div>
            ))}
